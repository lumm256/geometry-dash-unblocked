...
export function GameSEOStructuredData({ game }) {
  ...
  const structuredData = {
    ...
    "url": `https://geometry-dash-unblocked.com/games/${game.slug}/`,
...
export function BreadcrumbStructuredData({ items }) {
  ...
  const structuredData = {
    ...
    "itemListElement": items.map((item, index) => ({
      ...
      "position": index + 1,
      "name": item.label,
      "item": item.href ? `https://geometry-dash-unblocked.com${item.href}` : undefined
    }))
  };
...
export function GameMetaTags({ game }) {
  ...
  return (
    <>
      ...
      <link rel="canonical" href={`https://geometry-dash-unblocked.com/games/${game.slug}/`} />
    </>
  );
}
